...
public class Card {
	...
	public Card() {
		numbers = new int[5][5];
		int[] col1 = randomRange(1,15);
		int[] col2 = randomRange(16,30);
		int[] col3 = randomRange(31,45);
		int[] col4 = randomRange(46,60);
		int[] col5 = randomRange(61,75);
		for(int i = 0; i < 5; i++) {
			for(int j = 0; j < 5; j++) {
				if(i == 0) {
					numbers[j][i] = col1[j];
	...
	public boolean pulledNumber(int num) {
		if(num < 16) {
			for(int i = 0; i < 5; i++) {
				if(num == numbers[i][0]) {
					numbers[i][0] = 0;
